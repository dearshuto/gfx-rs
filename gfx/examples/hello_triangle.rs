use sjgfx::{wgpu::*, IApi, ApiWgpu};
use sjgfx::{
    CommandBufferBuilder, DeviceBuilder, QueueBuilder, SemaphoreBuilder, ShaderBuilder,
    SwapChainBuilder,
};
use sjgfx_interface::{PrimitiveTopology, DeviceInfo, QueueInfo, IDevice, IQueue};
//use sjgfx::vulkano::*;
...
fn run<TApi: IApi, TDevice: IDevice>() {
    let mut event_loop = EventLoop::new();
    let window = WindowBuilder::new().build(&event_loop).unwrap();

    let device = TApi::DeviceType::new(&DeviceInfo::new());
    let _queue = TApi::QueueType::new(&device, &QueueInfo::new());
