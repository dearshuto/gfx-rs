use sjgfx::{api::IApi, TShaderBuilder, TBufferBuilder, TTextureBuilder, TCommandBufferBuilder, TTextureViewBuilder, TSamplerBuilder, TVertexStateBuilder};
use sjgfx_interface::{IBuffer, ImageFormat, IQueue, ICommandBuffer, ITextureView, TextureViewInfo, PrimitiveTopology, IndexFormat, VertexAttributeStateInfo, AttributeFormat, VertexBufferStateInfo, ScissorStateInfo};
...
impl<TApi: IApi> RenderPass<TApi>
{
    pub fn new(device: &TApi::Device) -> Self {
        ...
        let vertex_shader_binary = shader_compiler.create_binary(include_str!("../resources/egui.vs"), sjgfx_util::ShaderStage::Vertex);
        let pixel_shader_binary = shader_compiler.create_binary(include_str!("../resources/egui.fs"), sjgfx_util::ShaderStage::Pixel);
        ...
        let constant_buffer = TBufferBuilder::<TApi>::new().enable_constant_buffer()
            .with_size(16)
            .build(device);
        ...
        let vertex_state = TVertexStateBuilder::<TApi>::new()
            ...
            .set_vertex_buffer_states([
                VertexBufferStateInfo::new()
                    .set_stride((std::mem::size_of::<f32>() * 2 + std::mem::size_of::<f32>() * 2 + std::mem::size_of::<u32>()) as i64)
            ].into_iter())
            .build(device);
        ...
        Self {
            ...
            texture: TextureCache { version: 0, texture_view, texture },
            sampler,
            constant_buffer: BufferCache{ size: 64, buffer: constant_buffer  },
            vertex_state,
        }
    }
    ...
    pub fn update_texture(&mut self, device: &TApi::Device, font_image: &egui::FontImage) {
        if self.texture.version == font_image.version {
            ...
            self.texture = TextureCache{ version: font_image.version, texture: texture, texture_view: texture_view  };
        }
    }

    pub fn update_user_textures(&mut self){}

    pub fn update_buffers(&mut self, device: &TApi::Device, paint_jobs: &[egui::epaint::ClippedMesh]) {
        for (index, egui::ClippedMesh(_, mesh)) in paint_jobs.iter().enumerate() {
            ...
            if index < self.index_buffers.len() {
                if let Some(buffer_cache) = Self::update_buffer(device, &mut self.index_buffers[index], data){
                    self.index_buffers[index] = buffer_cache;
                }
            } else {
                let buffer = TBufferBuilder::<TApi>::new().enable_index_buffer().with_size(data.len()).build(device);
                self.index_buffers.push(BufferCache{ size: data.len(), buffer });
            }

            // 頂点バッファの更新
            let data: &[u8] = bytemuck::cast_slice(&mesh.vertices);
            if index < self.vertex_buffers.len() {
                if let Some(buffer_cache) = Self::update_buffer(device, &mut self.vertex_buffers[index], data){
                    self.vertex_buffers[index] = buffer_cache;
                }
            } else {
                let buffer = TBufferBuilder::<TApi>::new().enable_vertex_buffer().with_size(data.len()).build(device);
                self.vertex_buffers.push(BufferCache{ size: data.len(), buffer });
            }
        }
    }

    pub fn execute(&mut self, color_target_view: &TApi::ColorTargetView, queue: &mut TApi::Queue, paint_jobs: &[egui::epaint::ClippedMesh], scale_factor: f32) {
            ...
            .zip(self.index_buffers.iter())
        {
            ...
            self.command_buffer.set_vertex_buffer(0, &vertex_buffer.buffer);
            self.command_buffer.set_vertex_state(&self.vertex_state);
            self.command_buffer.draw_indexed(PrimitiveTopology::TriangleList, IndexFormat::Uint32, &index_buffer.buffer, mesh.indices.len() as i32, 0);
    ...
    fn update_buffer(device: &TApi::Device, buffer_cache: &mut BufferCache<TApi>, data: &[u8]) -> Option<BufferCache<TApi>> {
        if data.len() > buffer_cache.size {
            ...
            Some(BufferCache{ size: data.len(), buffer })
        } else {
            // データを更新
