...
impl Compiler {
    ...
    pub fn build_graphics_shader<TPath: AsRef<Path>>(&mut self, vertex_shader_path: &TPath, pixel_shader_path: &TPath)
    {
        ...
        let vertex_shader_binary = self.compiler
            .compile_into_spirv(
            ...
            .unwrap();
        let pixel_shader_binary = self.compiler
            .compile_into_spirv(
                &pixel_shader_source,
        ...
        // 頂点シェーダ
        std::fs::File::create(output_directory_path.join(vertex_shader_file_path.file_name().unwrap()))
            .unwrap()
            .write_all(vertex_shader_binary.as_binary_u8())
            .unwrap();

        // ピクセルシェーダ
        std::fs::File::create(output_directory_path.join(pixel_shader_file_path.file_name().unwrap()))
            .unwrap()
