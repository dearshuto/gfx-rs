...
fn main()
{
	...
	let texture_memory_pool = sj::gfx::MemoryPool::new(&device,
													   &sj::gfx::MemoryPoolInfo::new()
													   .set_size(1024 * 1024)
													   .set_memory_pool_property(sj::gfx::MemoryPoolProperty::CPU_INVISIBLE));
	let texture_info = sj::gfx::TextureInfo::new().set_width(128).set_height(128).set_texture_usage(sj::gfx::TextureUsage::TEXTURE);
	let texture_required_size = sj::gfx::Texture::calculate_required_size(&device, &texture_info);
	let texture_memory_pool_offset = 0;
	let _texture = sj::gfx::Texture::new(&device, &texture_info, &texture_memory_pool, texture_memory_pool_offset, texture_required_size);
