...
impl<'a> ShaderImpl<'a>
{
	...
	fn create_descriptor_set_layout_bindings(shader_binary: &[u8]) -> Vec<ash::vk::DescriptorSetLayoutBinding>
	{
		...
		for set in bindings {
			match set.descriptor_type
			{
				...
				spirv_reflect::types::ReflectDescriptorType::StorageBuffer => storage_buffer_count += 1,
				_ => {}
			};
		}
...
{	
	fn new(device: &'a Device, info: &ShaderInfo) -> Self
	{
		...
		//let vertex_shader_module_create_info = ash::vk::ShaderModuleCreateInfo::builder()
			//.code(info.get_shader_binary())
		//.build();
		
		// let pixel_shader_module_create_info = ash::vk::ShaderModuleCreateInfo::builder()
		// 	//.code()
		// 	.build();
		...
		unsafe {
			...
			let compute_shader_module = device_impl.create_shader_module(&compute_shader_module_create_info, None).unwrap();
			let shader_modiles = [
			...
			let descriptor_set_layout_bindings = ShaderImpl::create_descriptor_set_layout_bindings(info.get_shader_binary());
			let descriptor_set_layout = device_impl.create_descriptor_set_layout(&ash::vk::DescriptorSetLayoutCreateInfo::builder()
																		  .bindings(descriptor_set_layout_bindings.as_slice())
																		  .build(), None).unwrap();
			let pipeline_layout = device_impl.create_pipeline_layout(&ash::vk::PipelineLayoutCreateInfo::builder()
			...
			let compute_pipeline_create_info = ash::vk::ComputePipelineCreateInfo::builder()
				...
				.build();
			let pipelines = device_impl.create_compute_pipelines(
