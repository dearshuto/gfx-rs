...
use super::super::{Device, ColorTargetView};
...
pub struct SwapChainImpl<'a> {
	...
	_layer: &'a mut Layer,
	_scan_buffer_views: Vec<ColorTargetView>,
}
...
impl<'a> ISwapChainImpl<'a> for SwapChainImpl<'a>
{
	...
	{
		let entry : &ash::EntryCustom<std::sync::Arc<libloading::Library>> = device.to_data().get_entry();
